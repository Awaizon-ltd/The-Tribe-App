...
import { useTribePosts } from '../../hooks/useTribePost';
...
const PostsScreen = ({ route, navigation }) => {
  const { tribeId, membershipStatus, tribe } = route.params;
  ...
  const onViewableItemsChanged = useCallback(({ viewableItems }) => {
    viewableItems.forEach(({ item }) => {
      if (item?.id && !impressionsSent.current.has(item.id)) {
        ...
        api.trackImpression(tribeId, item.id).catch(() => {});
      }
    });
  }, [tribeId]);
  ...
  const renderPost = ({ item }) => (
    <PostCard
      ...
      onNavigateTribe={
        navigation
          ? () => navigation.navigate('TribeDetail', { tribe })
          : undefined
      }
      onPressContent={
        navigation
          ? () => navigation.navigate('PostDetail', { post: item, tribeId, tribe })
          : undefined
      }
    />
  );
