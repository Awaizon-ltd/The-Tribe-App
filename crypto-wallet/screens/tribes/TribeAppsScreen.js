...
const TribeAppsScreen = ({ route, navigation }) => {
  const { tribeId, tribe, isOwner } = route.params;
  ...
  const handleUninstall = useCallback((miniApp) => {
    Alert.alert(
      'Remove App',
      `Remove ${miniApp.name} from this tribe?`,
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              await api.uninstallMiniApp(tribeId, miniApp.id);
        ...
        <View style={styles.emptyContainer}>
          ...
          {isOwner && (
            <Text style={styles.emptySubtext}>
              Tap + to browse and add mini-apps for this tribe.
            </Text>
          )}
        </View>
      ) : (
...
// ── Owner-only: browse global directory, install into this tribe ─────────────
const InstallPickerModal = ({ visible, onClose, tribeId, installedIds, onInstalled }) => {
  const { COLORS, SPACING, BORDER_RADIUS, FONTS } = useTheme();
  ...
  const handleInstall = async (app) => {
    ...
    try {
      ...
      await api.installMiniApp(tribeId, app.id, app.requestedScopes || []);
      onInstalled();
    } catch {
...
export default TribeAppsScreen;