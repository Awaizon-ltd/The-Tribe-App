...
import { useTribeSettings } from '../../hooks/useTribeSetting';
import { AddModeratorModal, ExternalLinksManager, InviteLinksManager, ModeratorsList } from '../../components/tribe/TribeSettingComponents';
import LinkDAOModal from '../../components/tribe/LinkDAOModal';
import api from '../../services/TribeApiService';
import Alert from '../../utils/Alert';

const TribeSettingsScreen = ({ route, navigation }) => {
  ...
  // Linked DAO state — seed from tribe prop, can be updated locally
  const [linkedDaoAddress, setLinkedDaoAddress] = useState(
    tribe.linkedDaoAddress || tribe.linked_dao_address || null,
  );
  const [linkedDaoChainId, setLinkedDaoChainId] = useState(
    tribe.linkedDaoChainId || tribe.linked_dao_chain_id || null,
  );
  ...
  const isOwner = tribe.created_by === user?.uid || tribe.createdBy === user?.uid;

  useEffect(() => {
    if (!isOwner) {
      Alert.alert('Access Denied', 'Only the tribe owner can access settings');
      navigation.goBack();
    }
  }, [isOwner]);

  const handleSaveTribeName = async () => {
    if (!newTribeName.trim() || newTribeName === tribe.name) {
    ...
    setSavingName(true);
    try {
      await updateTribeName(newTribeName.trim());
      Alert.alert('Success', 'Tribe name updated successfully');
      setEditingName(false);
    } catch (error) {
      Alert.alert('Error', error.message || 'Failed to update tribe name');
    } finally {
      setSavingName(false);
    }
  };
  ...
  const handleLinkDAO = async (daoAddress, chainId, daoName) => {
    try {
      const updated = await api.linkDao(tribe.id, daoAddress, chainId);
      ...
      Alert.alert('DAO Linked', `${daoName} is now linked to this tribe. A Governance tab will appear for all members.`);
    } catch (e) {
  ...
  const handleUnlinkDAO = () => {
    Alert.alert(
      'Unlink DAO',
      'This will remove the Governance tab from your tribe. Members will no longer see DAO proposals here.',
      [
        { text: 'Cancel', style: 'cancel' },
        {
          text: 'Unlink',
          ...
          onPress: async () => {
            ...
            try {
              await api.unlinkDao(tribe.id);
              setLinkedDaoAddress(null);
              setLinkedDaoChainId(null);
              setLinkedDaoName(null);
              Alert.alert('DAO Unlinked', 'The Governance tab has been removed from your tribe.');
            } catch (e) {
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={[styles.header, { paddingTop: insets.top + 8 }]}>
        ...
        <Text style={styles.headerTitle}>Tribe Settings</Text>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.section}>
          <View style={styles.sectionHeader}>
            ...
            <Text style={styles.sectionTitle}>Tribe Name</Text>
          </View>
