...
import AccessDeniedScreen from "../../components/tribe/AccessDeniedScreen";
...
const TribeDetailsScreen = ({ route, navigation }) => {
  const { tribe: tribeParam, tribeId: paramId } = route.params;
  const { COLORS, isDark } = useTheme();
  const insets = useSafeAreaInsets();
  const { user } = useAuth();
  const { address } = useWallet() || {};

  const [tribe, setTribe] = useState(tribeParam ?? null);
  // Block the UI only when we have no tribe object at all yet (deep-link / tribeId-only path).
  // When a tribe object is passed from the list we show it immediately and refresh silently.
  const [fetchingTribe, setFetching] = useState(!tribeParam && !!paramId);
  const [activeTab, setActiveTab] = useState("overview");
  const [members, setMembers] = useState([]);
  const [membersLoading, setMemberLoad] = useState(true);
  const [unreadCount, setUnreadCount] = useState(0);

  // Fade in when tribe loads
  const fadeAnim = React.useRef(new Animated.Value(0)).current;

  // ── Always fetch the full tribe object on mount ───────────────────────────────
  // Regardless of navigation source, we need the complete tribe record so that
  // fields like linked_dao_address (required for the Governance tab) are present.
  // When tribeParam was passed, the screen renders immediately from that data while
  // this fetch runs silently in the background — no loading spinner shown.
  useEffect(() => {
    const id = tribeParam?.id || paramId;
    if (!id) return;
    ...
    api
      ...
      .then((data) => {
        ...
        else if (!tribeParam) navigation.goBack(); // nothing to show — bail out
      })
      .catch(() => {
        if (!tribeParam) navigation.goBack();
      })
  ...
  const privacy = tribe?.privacy || "public";
  ...
  const {
    ...
    membershipStatus,
    joinLoading,
    handleJoinTribe: originalHandleJoin,
    setMemberCount,
  } = useTribeMembership(tribe || {}, user, privacy);

  // Pass token gating config from the tribe object so the hook skips a redundant API call.
  // Backend returns it as token_gating (snake_case) or tokenGating (camelCase).
  const tribeTokenGating = tribe?.token_gating ?? tribe?.tokenGating ?? null;
  ...
  useEffect(() => {
    ...
    const loadMembers = async () => {
      ...
      const { data: cached, stale } = await TribeCache.getMembers(tribe.id);
      ...
      try {
        const fresh = await api.getMembers(tribe.id);
        if (!mounted) return;
        setMembers(fresh || []);
        TribeCache.saveMembers(tribe.id, fresh || []).catch(() => {});
      } catch {
      } finally {
  ...
  // ── Unread count ─────────────────────────────────────────────────────────────
  useEffect(() => {
    if (!tribe?.id || !isMember || membershipStatus === "pending") return;
    let cancelled = false;

    const load = () => {
      api
        .getUnreadCount(tribe.id)
        .then((data) => {
  ...
  }, [tribe?.id, isMember, membershipStatus]);

  // ── Early returns (after all hooks) ──────────────────────────────────────────
  if (fetchingTribe || !tribe) {
    return (
  ...
  const isOwner =
    tribe.created_by === user?.uid || tribe.createdBy === user?.uid;
  ...
  const linkedDaoAddress = tribe.linkedDaoAddress || tribe.linked_dao_address;
  const linkedDaoChainId = tribe.linkedDaoChainId || tribe.linked_dao_chain_id;
  ...
  const handleTabPress = (tabId) => {
    if (tabId === "overview" || tabId === "request") {
      ...
      setUnreadCount(0);
      navigation.navigate("TribeChat", {
        tribeId: tribe.id,
        membershipStatus,
        members,
        logo: tribe.logoUrl || tribe.logo_url,
        name: tribe.name,
      });
    } else if (tabId === "posts") {
      navigation.navigate("TribePosts", {
        tribeId: tribe.id,
        membershipStatus,
        tribe,
      });
    } else if (tabId === "settings") {
      navigation.navigate("TribeSettings", { tribe });
    } else if (tabId === "governance") {
      navigation.navigate("TribeGovernance", {
    ...
    return (
      <View>
        ...
        <View style={s.bannerWrapper}>
          {tribe.bannerUrl || tribe.banner_url ? (
            <Image
              source={{ uri: tribe.bannerUrl || tribe.banner_url }}
              style={s.banner}
              resizeMode="cover"
            />
          ) : (
        ...
        <View style={s.infoRow}>
          <View style={[s.logoWrap, { borderColor: COLORS.background }]}>
            {tribe.logoUrl || tribe.logo_url ? (
              <Image
                source={{ uri: tribe.logoUrl || tribe.logo_url }}
                style={s.logo}
              />
            ) : (
              <View
          ...
          {!isMember && (
            ...
            >
              {joinLoading || tokenCheckLoading ? (
                ...
                <Text style={s.joinBtnText}>Join Tribe</Text>
              )}
            </TouchableOpacity>
          )}
        ...
        <View style={s.metaBlock}>
          <View style={s.nameLine}>
            <Text
              style={[s.tribeName, { color: COLORS.text }]}
              numberOfLines={1}
            >
              {tribe.name}
            </Text>
...
const styles = (COLORS) =>
  StyleSheet.create({
    ...
    tribeName: { fontSize: 22, fontWeight: "700", flex: 1 },
