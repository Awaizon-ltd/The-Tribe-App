...
const CreateTribeScreen = ({ navigation }) => {
  ...
  const [checkingTribeLimit, setCheckingTribeLimit]   = useState(true);
  const [userTribeCount, setUserTribeCount]           = useState(0);
  ...
  // ── Check how many tribes this user has already created ──────────────────────
  useEffect(() => {
    if (!user) return;

    const checkLimit = async () => {
      try {
        setCheckingTribeLimit(true);
        const tribes = await api.getMyTribes();
        const owned  = (tribes || []).filter(
          g => (g.createdBy || g.created_by) === user.uid,
        );
        const count = owned.length;
        setUserTribeCount(count);

        if (count >= MAX_TRIBES_PER_USER) {
          Alert.alert(
            "Tribe Limit Reached",
            `You can only create up to ${MAX_TRIBES_PER_USER} tribe. Delete an existing tribe to create a new one.`,
            [{ text: "Go Back", onPress: () => navigation.goBack() }],
            { cancelable: false },
          );
        }
      } catch (err) {
        console.error("[CreateTribe] tribe limit check failed:", err.message);
      } finally {
        setCheckingTribeLimit(false);
      }
    };
  ...
  const validate = () => {
    if (userTribeCount >= MAX_TRIBES_PER_USER) {
      Alert.alert("Tribe Limit Reached", `You can only create ${MAX_TRIBES_PER_USER} tribe.`);
      return false;
    }
    if (!name.trim())        { Alert.alert("Missing Info", "Please enter a tribe name.");   return false; }
    ...
    if (!logoUri)            { Alert.alert("Missing Info", "Please upload a tribe logo.");   return false; }
    if (!bannerUri)          { Alert.alert("Missing Info", "Please upload a tribe banner."); return false; }
  ...
  const handleCreateTribe = async () => {
    ...
    try {
      ...
      const logoResult = await uploadImageToCloudinary(
        logoUri, "tribe-images", { publicId: `tribe_logo_${Date.now()}` },
      );

      // Step 2 — upload banner
      setCreatingStep("banner");
      const bannerResult = await uploadImageToCloudinary(
        bannerUri, "tribe-images", { publicId: `tribe_banner_${Date.now()}` },
      );

      // Step 3 — create tribe record
      ...
      console.error("[CreateTribe] failed:", err.message);
      setLoading(false);
      setCreatingStep(null);
      Alert.alert("Error", err?.response?.data?.error || "Failed to create tribe. Please try again.");
    }
  };
  ...
  const styles = StyleSheet.create({
    ...
    tribeCount:          { color: COLORS.primary, fontSize: FONTS.sizes.sm, fontWeight: "600" },
    scrollView:          { flex: 1 },
  ...
  // ── Early returns ─────────────────────────────────────────────────────────────
  if (checkingTribeLimit) {
    return (
      <View style={styles.loadingContainer}>
        <ActivityIndicator size="large" color={COLORS.primary} />
        <Text style={styles.loadingText}>Checking tribe limit...</Text>
      </View>
    );
  }
  ...
  if (userTribeCount >= MAX_TRIBES_PER_USER) {
    return (
      <View style={styles.limitContainer}>
        ...
        <Text style={styles.limitTitle}>Tribe Limit Reached</Text>
        <Text style={styles.limitText}>
          You've created {userTribeCount} of {MAX_TRIBES_PER_USER} tribe.
        </Text>
        <Text style={styles.limitSubtext}>Delete an existing tribe to create a new one.</Text>
      ...
      >
        <View style={[styles.header, { paddingTop: insets.top + 8 }]}>
          ...
          <View style={styles.headerRight}>
            <Text style={styles.tribeCount}>{userTribeCount}/{MAX_TRIBES_PER_USER}</Text>
          </View>
        </View>
        ...
        {(() => {
          const STEPS = [
            { key: "logo",     icon: "image",         title: "Uploading Logo",    sub: "Uploading your tribe logo..." },
            { key: "banner",   icon: "image-outline", title: "Uploading Banner",  sub: "Uploading your tribe banner..." },
            { key: "creating", icon: "hammer",         title: "Creating Tribe",    sub: "Setting up your tribe on-chain..." },
            { key: "done",     icon: "checkmark-circle", title: "Tribe Created!",  sub: "Your tribe is ready." },
          ];
          ...
          <View style={styles.modalOverlay}>
            <View style={styles.modalContent}>
              ...
              <View style={styles.privacyModalContent}>
                {[
                  { value: "public",  icon: "globe",       color: COLORS.primary, title: "Public Tribe",   desc: "Anyone can join without restrictions" },
                  { value: "private", icon: "lock-closed", color: "#FFD700",       title: "Private Tribe",  desc: "Token gated — members must hold specific tokens/NFTs" },
                ].map((opt) => (
                  <TouchableOpacity
                    key={opt.value}
