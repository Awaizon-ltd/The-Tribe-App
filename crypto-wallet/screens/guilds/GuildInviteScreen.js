...
const InviteScreen = ({ route }) => {
  ...
  // Initialize hooks after guild data is loaded
  const guildMembership = guildData
    ? useGuildMembership(guildData, user, guildData.privacy || "public")
    : null;

  const tokenGating = guildData
    ? useTokenGating(
        guildData.id,
        user,
        address,
        guildData.privacy || "public",
        guildMembership?.isMember
      )
    : null;
  ...
  const fetchInviteData = async () => {
    ...
    try {
      ...
      // Fetch the guild the invite belongs to
      const guild = await api.getGuild(invite.guild_id);

      if (!guild) {
        setError("guild_not_found");
        setLoading(false);
        return;
      }
  ...
  const handleJoinGuild = async () => {
    if (!guildData || !inviteData) return;

    // Token gating check
    if (guildData.privacy === "private" && tokenGating?.tokenGatingData && !tokenGating?.hasRequiredToken) {
      ...
      Alert.alert("Access Denied", `You need to hold ${req} to join this guild.`);
      return;
    }
    ...
    try {
      ...
      await api.joinGuild(guildData.id, {
        username:      user?.displayName || user?.email?.split("@")[0] || "Unknown",
        walletAddress: address || null,
        inviteId:      inviteData.id,
      });

      setTimeout(() => navigation.replace("GuildDetail", { guild: guildData }), 500);
    } catch (err) {
        ...
        >
          <View style={styles.errorContainer}>
            ...
            <Text style={styles.errorMessage}>
              {error === "authentication" &&
                "Please log in to accept guild invites."}
              {error === "invalid" &&
              ...
              {error === "guild_not_found" &&
                "The guild associated with this invite could not be found."}
              {error === "fetch_error" &&
  ...
  // Success State - Show Guild Info
  const isMember    = guildMembership?.isMember || false;
  const memberCount = guildMembership?.memberCount || guildData?.member_count || guildData?.memberCount || 0;
    ...
    <View style={styles.container}>
      ...
      >
        ...
        >
          ...
          {guildData?.bannerUrl && (
            <Image
              source={{ uri: guildData.bannerUrl }}
              style={styles.banner}
              resizeMode="cover"
            />
          )}
          ...
          <View style={styles.logoContainer}>
            <Image source={{ uri: guildData?.logoUrl }} style={styles.logo} />
            {guildData?.privacy === "private" && (
          ...
          <View style={styles.infoSection}>
            <Text style={styles.title}>{guildData?.name}</Text>

            <View style={styles.genreContainer}>
              <View style={styles.genreBadge}>
                <Text style={styles.genreText}>{guildData?.genre}</Text>
              </View>
            </View>

            {guildData?.description && (
              <Text style={styles.description}>{guildData.description}</Text>
            )}
                  ...
                  >
                    ...
                    >
                      <Ionicons
                        ...
                        color={COLORS.card || "#FFFFFF"}
                      />
                      <Text style={styles.primaryButtonText}>Go to Guild</Text>
                    </LinearGradient>
                  </TouchableOpacity>
                </>
              ) : (
