...
const CommentItem = ({ comment, COLORS }) => {
  const ts = typeof comment.createdAt === 'number' ? comment.createdAt : Date.now();
    ...
    <View style={ci.row}>
      ...
      <View style={[ci.bubble, { backgroundColor: COLORS.surface }]}>
        <View style={ci.header}>
          ...
          <Text style={[ci.time, { color: COLORS.textTertiary }]}>{timeAgo(ts)}</Text>
        </View>
...
const PostDetailScreen = ({ route, navigation }) => {
  // Deep links pass only { postId, guildId }; normal navigation passes a full post object.
  const { post: initialPost, postId: deepLinkPostId, guildId, guild } = route.params;
  ...
  useEffect(() => {
    if (initialPost || !deepLinkPostId || !guildId) return;
    setFetchingPost(true);
    api.getPost(guildId, deepLinkPostId)
      .then(data => setPost(data))
      .catch(() => {})
      .finally(() => setFetchingPost(false));
  }, [deepLinkPostId, guildId]);
  ...
  const logoUrl   = guild?.logo_url || guild?.logoUrl || post.guildLogoUrl;
  const guildName = guild?.name || post.guildName || 'Guild';
  const resolvedGuildId = guildId || post.guildId;

  // ── Load comments from MongoDB via backend API ───────────────────────────
  useEffect(() => {
    if (!resolvedGuildId || !post.id) return;
    let cancelled = false;

    setLoadingComments(true);
    api.getComments(resolvedGuildId, post.id, 100, 0)
      .then(data => {
        if (!cancelled) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const saved = await api.addComment(resolvedGuildId, post.id, { text, username, userAvatar });
      setCommentText('');
    ...
    <View>
      ...
      <View style={s.authorRow}>
        ...
        <View style={s.authorText}>
          <Text style={[s.guildName, { color: COLORS.text }]} numberOfLines={1}>{guildName}</Text>
