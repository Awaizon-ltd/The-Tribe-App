...
export const useGuildChat = (guildId) => {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      try {
        const cached = (await getCachedMessages(guildId, PAGE_SIZE)).map(norm);
      ...
      try {
        const localSettings = await dbGetSettings(guildId);
      ...
      try {
        ...
        if (mounted) setIsAdmin(guild?.created_by === user.uid || guild?.createdBy === user.uid);
      } catch {}
  ...
  // ── Polling ───────────────────────────────────────────────────────────────────
  useEffect(() => {
    if (!user || !guildId || loading) return;

    const poll = async () => {
      if (!isActiveRef.current) return;
      try {
        const fresh = await api.pollMessages(guildId, latestTs.current);
        if (fresh?.length > 0) {
          const normed = fresh.map(norm);
          setMessages(prev => merge(prev, normed));
          ...
          if (newest > latestTs.current) {
            ...
            updateSyncState(guildId, newest, oldestTs.current).catch(() => {});
          }
          batchCacheMessages(guildId, normed).catch(() => {});
        }
      } catch { /* non-fatal */ }
  ...
  const loadMoreMessages = useCallback(async () => {
    ...
    try {
      ...
      const cached = (await getCachedMessages(guildId, PAGE_SIZE, before)).map(norm);
      ...
      const apiOld = await api.getMessages(guildId, PAGE_SIZE, before);
      if (!apiOld?.length) {
  ...
  const sendMessage = useCallback(async (text, replyTo = null) => {
    ...
    try {
      const res  = await api.sendMessage(guildId, {
        text:        text.trim(),
        username:    userData?.username    || user.email?.split('@')[0] || 'Unknown',
        displayName: userData?.displayName || null,
        userAvatar:  userData?.profilePicture || null,
      ...
      cacheMessage(guildId, real).catch(() => {});
      updateSyncState(guildId, latestTs.current, oldestTs.current).catch(() => {});
    } catch (err) {
  ...
  }, [guildId, user, userData, chatSettings, isAdmin]);
  ...
  const editMessage = useCallback(async (messageId, newText) => {
    ...
    updateCachedMessage(guildId, messageId, { text: newText.trim(), isEdited: true, editedAt: now })
      .catch(() => {});

    await api.editMessage(guildId, messageId, newText);
  }, [guildId, user, messages]);

  // ── Delete (optimistic) ───────────────────────────────────────────────────────
  const deleteMessage = useCallback(async (messageId) => {
    setMessages(prev => prev.filter(m => m.id !== messageId));
    deleteCachedMessage(guildId, messageId).catch(() => {});
    await api.deleteMessage(guildId, messageId);
  }, [guildId]);
  ...
  const deleteAllUserMessages = useCallback(async (userId) => {
    ...
    deleteCachedUserMessages(guildId, userId).catch(() => {});
    await api.deleteAllUserMessages(guildId, userId);
  }, [guildId, isAdmin]);

  // ── Ban ───────────────────────────────────────────────────────────────────────
  const banUser = useCallback(async (userId, username) => {
    if (!isAdmin) throw new Error('Only admins can ban users');
    await api.banUser(guildId, userId, username);
    setMessages(prev => prev.filter(m => m.userId !== userId));
    deleteCachedUserMessages(guildId, userId).catch(() => {});
  }, [guildId, isAdmin]);
