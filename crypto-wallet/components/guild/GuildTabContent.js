...
import GuildRequestsScreen from "../../screens/guilds/GuildRequestScreen";
import PostsScreen from "../../screens/guilds/GuildPostScreen";
import TokenRequirements from "./TokenRequirements";
import {
  getExternalLinks,
  getGuildInvites,
} from "../../utils/GuildManagementDB";
import Alert from "../../utils/Alert";

const GuildTabContent = ({
  activeTab,
  guild,
  memberCount,
  members,
  ...
  }, [activeTab, guild.id, isMember]);

  const loadOverviewData = async () => {
    setLoadingLinks(true);
    try {
      // Load external links
      const links = await getExternalLinks(guild.id);
  ...
  const handleCopyInvite = async () => {
    ...
    const inviteUrl = `https://thetribe.com/invite/${activeInvite.code}`;
  ...
  const handleShareInvite = async () => {
    ...
    const inviteUrl = `https://thetribe.com/invite/${activeInvite.code}`;
    await Share.share({
      message: `Join ${guild.name} on our app!\n${inviteUrl}`,
    });
  };
    ...
    <View style={styles.tabContent}>
      ...
      >
        ...
        <View style={styles.statItem}>
          ...
          />
          <Text style={[styles.statNumber, { color: COLORS.text }]}>
            {guild.createdAt?.toDate
              ? new Date(guild.createdAt.toDate()).getFullYear()
      ...
      {/* Token Requirements (Private Guilds) */}
      ...
      <View style={styles.aboutSection}>
        ...
        <Text style={[styles.aboutText, { color: COLORS.textSecondary }]}>
          {guild.description || "No description provided for this guild."}
        </Text>
      </View>
  ...
  const renderRequestTab = () => {
    if (membershipStatus === "owner" && privacy === "private") {
      return (
        <View style={styles.tabContent}>
          <GuildRequestsScreen guildId={guild.id} />
        </View>
      );
    }
    return (
